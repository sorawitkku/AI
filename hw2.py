...
def minimax(board, count_list, my_piece, N, M, X, depth, maximizing):
    terminal, winner = is_game_over(board, my_piece if maximizing else oppo_piece, N, M, X)
    if terminal:
        if winner == my_piece:
            return 10000 - depth
        elif winner == oppo_piece:
            return -10000 + depth
        else:
            return 0
    if depth == 0:
        return scoring_method(board, my_piece, N, M, X)
    
    if maximizing:
        max_eval = -math.inf
        for move in all_possible_moves(count_list, N, M):
            if count_list[move] != N:
                sim_board = board.copy()
                update_state(sim_board, move, my_piece, count_list)
                count_list[move] += 1
                eval = minimax(sim_board, count_list, my_piece, N, M, X, depth - 1, False)
                count_list[move] -= 1
        ...
        for move in all_possible_moves(count_list, N, M):
            if count_list[move] != N:
                sim_board = board.copy()
                update_state(sim_board, move, oppo_piece, count_list)
                count_list[move] += 1
                eval = minimax(sim_board, count_list, my_piece, N, M, X, depth - 1, True)
                count_list[move] -= 1
...
def best_move(board, count_list, my_piece, N, M, X):
    ...
    for move in all_possible_moves(count_list, N, M):
        if count_list[move] != N:
            ...
            update_state(sim_board, move, my_piece, count_list)
            count_list[move] += 1
            score = minimax(sim_board, count_list, my_piece, N, M, X, 3, False)
            count_list[move] -= 1
            if score > best_score:
                best_score = score
                best_move = move
